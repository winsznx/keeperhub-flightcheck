...
| # | Claim | Level | Evidence |
|---|---|---|---|
| 39 | The interactive key prompt echoes nothing | live pty | **This claim was false in the first release of the extension.** An external audit drove the real CLI under a kernel pty on macOS with Node 24 and the key printed in plaintext. Echo is now suppressed with `stty` and the terminal's actual state is verified before a byte is read; re-tested under the same pty harness, no echo. `agent/tests/pty-echo.test.ts` | The test itself was flaky and was fixed on 2026-08-12: it typed on a fixed 2s timer, so a slow reachability call meant the key went in before the prompt existed and the terminal echoed it, which the test reported as a leak. It now waits for the prompt and distinguishes a real echo from its own race. Mutation-tested: removing both suppression layers makes it fail, removing either alone does not, and `evidence/probes/pty-echo-mutation.md` records why.
